...
def chunk_history_file(filepath):
    ...
    for line in lines:
        if line.startswith('## ') or line.startswith('### '):
            ...
            section_title = line.lstrip('#').strip(); current_section = line + "\n"
...
def chunk_source_files(project_path):
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        for file in files:
            ...
            filepath = os.path.join(root, file); rel_path = os.path.relpath(filepath, project_path)
            try:
...
def index_project(project_path):
    index_dir = os.path.join(project_path, '.ai-index'); os.makedirs(index_dir, exist_ok=True)
    ...
    history_chunks = chunk_history_file(history_path); bugs_chunks = chunk_history_file(bugs_path)
    all_history_chunks = history_chunks + bugs_chunks; code_chunks = chunk_source_files(project_path)

    if provider == 'qdrant':
        if QdrantClient is None: print("❌ error: qdrant-client not installed."); return
        ...
        for col in [col_history, col_code]:
            if client.collection_exists(collection_name=col): client.delete_collection(collection_name=col)
            client.create_collection(collection_name=col, vectors_config=qmodels.VectorParams(size=768, distance=qmodels.Distance.COSINE))
        ...
        if all_history_chunks:
            try: client.delete_collection("project_history")
            except: pass
            history_col = client.create_collection("project_history")
        ...
        if code_chunks:
            try: client.delete_collection("source_code")
            except: pass
            code_col = client.create_collection("source_code")
            batch_size = 100
